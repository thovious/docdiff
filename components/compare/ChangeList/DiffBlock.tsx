      ...
      {change.inlineDiff.length > 0 && (
        <div className="rounded-md border border-gray-200 bg-gray-50 p-3 font-mono text-xs leading-relaxed">
          ...
          {change.inlineDiff.map((seg, i) => {
            if (seg.type === 'equal') {
              return <span key={i} className="text-gray-700">{seg.value}</span>;
            }
            if (seg.type === 'added') {
              return (
                <mark key={i} className="rounded bg-green-200 px-0.5 text-green-900 not-italic">
                  {seg.value}
                </mark>
              );
            }
            return (
              <mark key={i} className="rounded bg-red-200 px-0.5 text-red-900 line-through not-italic">
                {seg.value}
              </mark>
            );
          })}
